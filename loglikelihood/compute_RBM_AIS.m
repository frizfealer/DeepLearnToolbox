...
if(nargin>5)
    ...
    log_base_rate = compute_logBaseRate( batchdata );
...
%B_k = 0, reference the last equation in page 43.
logww  =  logww - (negdata*visbiases_base' + numhids*log(2));

Wh = negdata*vishid + hidbias; %biase from the RBM model
Bv_base = negdata*visbiases_base'; %bias from the base rate
Bv = negdata*visbiases'; %bias from the RBM model
tt=1;

%%% The CORE of an AIS RUN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%reference the last equation in page 43 and equation 4.10
%logww: log of important weight
for bb = beta(2:end-1)
    if verbose == 1
